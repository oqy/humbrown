...
/**
 * 自定义SqlSessionFactoryBean，自动注册整形及字符型DescribableEnum枚举类的typeHandler，
 * 但会忽略已设定的config配置文件信息
 * 
 * @author qingyong_ou
 */
public class CustomSqlSessionFactoryBean extends SqlSessionFactoryBean {
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (hasLength(this.describableEnumPackage)) {
			...
			String[] describableEnumPackages = tokenizeToStringArray(this.describableEnumPackage,
			...
			for (String packageToScan : describableEnumPackages) {
				...
				for (Class<? extends DescribableEnum<?>> clazz : typeSet) {
					sb.append("<typeHandler handler=\"" + DescribableEnumTypeHandler.class.getName() + "\" javaType=\""
							+ clazz.getName() + "\"/>");
				}
			}
