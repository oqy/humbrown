...
public class MySqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>,
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		// 注册Model对象转换器别名
		configuration.getTypeAliasRegistry().registerAlias(ModelTypeHandler.class);
		// 注册PermissionInfo对象别名
		configuration.getTypeAliasRegistry().registerAlias(PermissionInfo.class);
		...
		if (hasLength(this.coreEnumPackage)) {
			String[] coreEnumPackageArray = tokenizeToStringArray(
					this.coreEnumPackage,
					ConfigurableApplicationContext.CONFIG_LOCATION_DELIMITERS);
					...
					if (ICoreEnum.class.isAssignableFrom(clazz)
						...
						if (type instanceof ParameterizedType) {
							...
							if (parameterizedType[0] == String.class) {
								configuration.getTypeHandlerRegistry().register(clazz,new StringEnumTypeHandler(clazz));
							} else if (parameterizedType[0] == Integer.class) {
								configuration.getTypeHandlerRegistry().register(clazz,new IntEnumTypeHandler(clazz));
							}
						}
					}
				}
