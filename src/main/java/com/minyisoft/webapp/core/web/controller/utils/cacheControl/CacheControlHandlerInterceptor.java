package com.minyisoft.webapp.core.web.controller.utils.cacheControl;
...
public class CacheControlHandlerInterceptor extends HandlerInterceptorAdapter
	...
	protected final void assignCacheControlHeader(
		...
		final CacheControl cacheControl = this.getCacheControl(request,
				response, handler);
		final String cacheControlHeader = this
		...
		if (cacheControlHeader != null) {
			...
			if (useExpiresHeader) {
				response.setDateHeader(HEADER_EXPIRES,
						createExpiresHeader(cacheControl));
			}
		}
	}
	 ...
	 * 
	 * @param cacheControl
	 *            the <code>CacheControl</code> annotation from which to create
	 *            the returned cache control header value
	 * 
	 * @return the cache control header value
	 */
	protected final String createCacheControlHeader(
	 ...
	 * @param cacheControl
	 *            the <code>CacheControl</code> annotation from which to create
	 *            the returned expires header value
	 * 
	...
	protected final CacheControl getCacheControl(
		...
		CacheControl cacheControl = handlerMethod
				.getMethodAnnotation(CacheControl.class);

		if (cacheControl == null) {
			return handlerMethod.getBeanType()
					.getAnnotation(CacheControl.class);
		}
	 ...
	 *            <code>true</code> to set an expires header when a
